...
if ($query)
{
  ...
  catch (Exception $e)
  {
    ...
    die("<html><head><title>SEARCH EXCEPTION</title><body><pre>{$e->__toString()}</pre></body></head></html>");
  }
}
...
<!--
    ...
    $(function() {
        ...
        $.ajax({
          url: "http://localhost:8983/solr/myexample/suggest?q=" + userInput + "&wt=json",
          //url : customURL,
          dataType: 'jsonp',
          'jsonp': 'json.wrf',
          data:{
          ...
          success : function(data) {
            var last=$("#q").val().toLowerCase().split(" ").pop(-1);
            suggestions =[]
            data.suggest.suggest[last].suggestions.forEach(function(value){
              var prefix = "";
              var query=$("#q").val();
              var queries=query.split(" ");
              if(queries.length>1) {
                var lastIndex=$("#q").val().trim().lastIndexOf(" ");
                prefix=query.trim().substring(0,lastIndex+1).toLowerCase();
              }
             ...
             suggestions.push(prefix+value.term)
            });
...
console.log("inside script");
   $(function() {
    ...
    <form accept-charset="utf-8" method="get" class = "form-group">
      <br>
      <label for="q" id="q"><i><font color = blue>SEARCH</i></font></label>
      <input id="q" class="form-control form-rounded" name="q" type="text" value="<?php echo htmlspecialchars($original,ENT_QUOTES,'utf-8'); ?>"/>
      <br>
...
<?php
  ...
  foreach ($results->response->docs as $doc)
  {
    ...
    // if(trim($snipped_Data) == "" && preg_match('/' . $phrase_joined . '/i', $doc->og_description)) 
    // {
    //   $snipped_Data = $doc->og_description;
    // }
    // else if(trim($snipped_Data) == "" && preg_match('/' . $phrase_joined . '/i', $doc->title)) 
    // {
    //   $snipped_Data = $doc->title;
    // }
    // if(!preg_match('/' . $phrase_joined . '/i', $snipped_Data)) 
    // {
    //   $snipped_Data = "";
    // }
