...
for key in dict_of_states:

    if key in us_state_abbrev_2:
        new_dict = {}
        new_dict["state"] = key
        new_dict["shootings"] = dict_of_states[key]
        new_dict["ufos"] = 0
        print(new_dict)
...
print (dict_of_ufo_states)
for key in dict_of_ufo_states:
    for curr in states_list:

        if key in states_set:
            if curr ["state"] == key:
                curr["ufos"] = dict_of_ufo_states[key]
        else:
            ...
            if key in us_state_abbrev_2:
                ...
                new_dict["ufos"] = dict_of_ufo_states[key]
                print(new_dict)
                states_list.append(new_dict)
                states_set.add(key)
...
for curr in states_list:
    ...
    try:
        curr["state"] = us_state_abbrev_2[curr['state']]
    except:
        err = str(index) + " " + str(e)
        print(err)
        pass
...
if os.path.exists('visualization_4.json'):
    os.remove('visualization_4.json')

with open('visualization_4.json', 'w') as outfile:
